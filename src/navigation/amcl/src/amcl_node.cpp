        ...
        first_reconfigure_call_(true)
{
  ...
  pose_pub_no_cov = nh_.advertise<geometry_msgs::PoseStamped>("amcl_pose_no_cov", 2, true);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      // Copy in the pose
      p2.pose.position.x = p.pose.pose.position.x;
      p2.pose.position.y = p.pose.pose.position.y;
      p2.pose.orientation = p.pose.pose.orientation;
 
      pose_pub_no_cov.publish(p2);
