...
int main(int argc, char **argv)
{
	...
	if((int(cur_time - last_time)) >= 100000)
	  ...
	  last_time = cur_time;
	  //sp3operation();
	  //getOdomPose(ss.location[0] , ss.location[1] , ss.location[2] , listener , transform);
          //ROS_INFO("x: %lf, y: %lf, yaw: %lf ",ss.location[0],ss.location[1],ss.location[2]);
            ...
            if(ss.save_end1>=ADV_MSG_LENGTH)
            {
                for(i=0;i<=ss.save_end1-ADV_MSG_LENGTH;i++)
                {
                    if(ss.rcv_buff_save1[i]==0x10&&(unsigned char)ss.rcv_buff_save1[i+1]==0x82)
                    {
                        ...
                        {
            			    ss.ADVnum=ss.rcv_buff_save1[i+2];
            			    ss.left_enc=int(ss.rcv_buff_save1[i+3]);
            			    ss.right_enc=int(ss.rcv_buff_save1[i+4]);
            			    ss.ADVstatus=ss.rcv_buff_save1[i+5];
            			    if(ss.rcv_buff_save1[i+6]==0x02)
					{
					   ss.lamp_time = 0;
					}
                                    ss.speedflag=ss.rcv_buff_save1[i+7];
               			    ss.powerstate=ss.rcv_buff_save1[i+8];
			                //ROS_INFO("status: %x,%x,%x,%x ",ss.ADVnum,ss.ADVstatus,ss.left_enc,ss.right_enc);
                            ss.save_end1=0;

                            
                            /*//The first place change here
                             ss.ADVnum=ss.rcv_buff_save1[i+2];
                             ss.left_enc=int(ss.rcv_buff_save1[i+3]);
                             ss.right_enc=int(ss.rcv_buff_save1[i+4]);
                             ss.ADVstatus=ss.rcv_buff_save1[i+5];
                             ss.speedflag=ss.rcv_buff_save1[i+8];
                            // ROS_INFO("status: %x,%x,%x,%x ",ss.ADVnum,ss.ADVstatus,ss.speedflag,ss.powerstate);
                             ss.save_end1=0;*/
                        }
                    } 
                }
...
void sp3operation()
{
    ...
    ss.buff[1]=0x13;
    ss.buff[2]=ss.ADVnum;
    ss.buff[3]=ss.lamp_color;
    ss.buff[4]=ss.lamp_time;
    ss.buff[5]=ss.ADVstatus;
    ss.buff[6]=0x00;
    ss.buff[7]=ss.speedflag;
    ss.buff[8]=ss.powerstate;
    ss.buff[9]=bcc_check(ss.buff,9);
    ss.buff[10]=0x03;
    write(ss.sp3.return_port(), ss.buff, ADV_MSG_LENGTH);

    ss.buff[0]=0x20;
    ss.buff[1]=0x10;
    ss.buff[2]=ss.ADVnum;
    ss.buff[3]=high_two(ss.location[0]);
    ss.buff[4]=low_two(ss.location[0]);
    ss.buff[5]=high_two(ss.location[1]);
    ss.buff[6]=low_two(ss.location[1]);
    ss.buff[7]=high_two(ss.location[2]);
    ss.buff[8]=low_two(ss.location[2]);
    ss.buff[9]=bcc_check(ss.buff,9);
    ss.buff[10]=0x03;
    write(ss.sp3.return_port(), ss.buff, ADV_MSG_LENGTH);
    ...
    FD_ZERO(&ss.rd3);
    FD_SET(ss.sp3.return_port(),&ss.rd3);
    struct timeval tv;
    tv.tv_sec = 0;
    tv.tv_usec = 10000;
    //fd method to read sp
    //while(FD_ISSET(ss.sp.return_port(),&ss.rd3))
    int retval=0;
    retval=select(ss.sp3.return_port()+1,&ss.rd3,NULL,NULL,&tv);
    if(retval < 0)
        perror("select error!\n");
    else
    {
        if(retval && FD_ISSET(ss.sp3.return_port(),&ss.rd3))
        {
            int s3_recv_len=read(ss.sp3.return_port(),ss.rcv_buff3,MAXSIZE);
            ROS_INFO("get: %x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x",ss.rcv_buff3[0],ss.rcv_buff3[1],ss.rcv_buff3[2],ss.rcv_buff3[3],ss.rcv_buff3[4],ss.rcv_buff3[5],ss.rcv_buff3[6],ss.rcv_buff3[7],ss.rcv_buff3[8],ss.rcv_buff3[9],ss.rcv_buff3[10]);

            for(i=0; i<s3_recv_len; i++ ){
                ss.rcv_buff_save3[ss.save_end3]=ss.rcv_buff3[i];
                ss.save_end3++;
                if(ss.save_end3>=MAXSIZE)
                   ss.save_end3=0;

            }
            if(ss.save_end3>=ADV_MSG_LENGTH)
            {
                for(i=0;i<=ss.save_end3-ADV_MSG_LENGTH;i++)
                {
                    if(ss.rcv_buff_save3[i]==0x10&&ss.rcv_buff_save3[i+1]==0x71)
                    {
			if(bcc_check(ss.rcv_buff_save3,i,ADV_MSG_LENGTH-2)==ss.rcv_buff_save3[i+ADV_MSG_LENGTH-2])
                        {    
			    for(int j=0;j<ADV_MSG_LENGTH;j++)
			    {
				ss.buff[j]=ss.rcv_buff_save3[i+j];
			    }
			    write(ss.sp1.return_port(), ss.buff, ADV_MSG_LENGTH);
			    ROS_INFO("send out");
...
int calculate_odom(nav_msgs::Odometry & odom)
{
    ...
    float ticks_per_meter=1259.5976;   //设定多少个脉冲前进一米
    float wheel_track=0.32;          //轮间距
